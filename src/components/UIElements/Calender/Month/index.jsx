...
const Month = ({month, year, today, SelectDate, bookingDate }) => {
...
const selectedLastDateIndex = (bookingDate.length-1)

  if(bookingDate.length > 0){
    if(bookingDate.indexOf(dayIntVal) === 0){
      if(bookingDate.length === 1){
        return 'only-booked'
      }else{
        return 'start-date'
      }
    }else if(bookingDate.indexOf(dayIntVal) === 1){
      return 'end-date'
    } else if(     bookingDate.length === 2
                && dayIntVal > bookingDate[0] 
                && dayIntVal < bookingDate[selectedLastDateIndex]){
      return 'mid-date'          
    }else{
      return 'not-selected'
    }
