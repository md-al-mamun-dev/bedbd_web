...
const GuestCount = ({guestCount, setGuestCount, guestCounterRef, show, changeGuestCount }) => {
  ...
  const increment = debounceCount(
    (clickCount, key='') => {
      if( key !=='adult' && guestCount['adult'] < 1){
        changeGuestCount('adult', 1)
        changeGuestCount(key,  clickCount)
        
        // setGuestCount({...guestCount, 'adult': 1, [key]: clickCount})  
      }else{
        // setGuestCount({...guestCount, [key]: (guestCount[key] + clickCount)})
  
        changeGuestCount(key,  guestCount[key] + clickCount)
      }
    }
  )
  const decrement = debounceCount(
    (clickCount, key='') => {
        ...
        if(guestFieldChangedValue > 0 ){
          ...
          changeGuestCount(key, guestFieldChangedValue)     

        }
        else if(key === 'adult' && (guestCount['children'] + guestCount['infants'] + guestCount['pets'])>0 )
          {  
            // setGuestCount({...guestCount, [key]: 1})
            changeGuestCount(key,  1)
          }
        else {
          // setGuestCount({...guestCount, [key]: 0})
          changeGuestCount(key,  0)
        }
      }
  )

  return (
    <div ref={guestCounterRef} className={`${styles.guests_count} left-50 translateX-50 top-179px z-index-12 ${show ? 'display-static' : 'display-none' }`}>
