...
export default function AddBookingType({data, closeModal}) {
   ...
   function onAddBtnClickHandlar(e){
        ...
        if(title.length>3 && title.length < 100 && description.length>3 && description.length < 300 ){
            // if(data.length>0){
                ...
                const id = uuidv4()
                dispatch({type:'property/addBookingType', data:{id:`new_type_${id}` , title, description}})
                dispatch({type:'property/addSelectedBookingType', data: `new_type_${id}`})
            // }else{
